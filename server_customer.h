...
void view_balance(int socket, struct Customer *customer) {
    send(socket, &customer->balance, sizeof(customer->balance), 0);
    printf("Sent balance of %d to user %s\n", customer->balance, customer->username);
}

void deposit_money(int sock, struct Customer *customer) {
    char line[BUFFER_SIZE];
    int deposit, found = 0;

    read(sock, &deposit, sizeof(deposit));
    ...
    while ((bytes_read = read(file, line, sizeof(line) - 1)) > 0) {
        ...
        char *new_pos = strchr(line, '\n');
        if (new_pos) {
            int line_length = new_pos - line + 1;
            ...
            // if no new line found move pointer forward
